...
module.exports = {
    displayProduct: (req, res) => {
        Product.findAll({
            ...
            { model: Unit }, { model: Product_Shape, include: { model: Shape } }, { model: Product_Flavour, include: { model: Flavour } }]
        }).then((result) => {
            res.send({
                success: true,
                productlist: result
    ...
    createProduct: (req, res) => {
        ...
        checkProduct(req.body.name).then((result) => {
            // console.log(req.body.flavourlist.length);
            if (result === 1) {
                if (req.body.imagebase) {
                    base64Img.img(req.body.imagebase, 'uploads', req.body.name.toLowerCase().split(" ").join(''), function (err, filepath) {
                ...
                Product.create(req.body).then((result) => {
                    ...
                    if (req.body.flavourid.length > 0) {
                        req.body.flavourid.forEach((specification, i) => {
                            flavourlist.push({ product_id: result.id, flavour_id: specification })
                        })
                        req.body.shapeid.forEach((shape, i) => {
                            shapelist.push({ product_id: result.id, shape_id: shape })
                        })
                        Promise.all([Product_Flavour.bulkCreate(flavourlist), Product_Shape.bulkCreate(shapelist)]).then((result1) => {
                            res.send({
                                success: true,
        ...
        if (req.body.imagebase) {
            base64Img.img(req.body.imagebase, 'uploads/', req.body.name.toLowerCase().split(" ").join(''), function (err, filepath) {
                ...
                Product.update(req.body, {
                    ...
                    if (req.body.flavourid.length > 0) {
                        req.body.flavourid.forEach((specification, i) => {
                            flavourlist.push({ product_id: req.body.id, flavour_id: specification })
                        })
                        req.body.shapeid.forEach((shape, i) => {
                    ...
                    Promise.all([Product_Flavour.destroy({
                        where: {
                            product_id: req.body.id
                        }
                    }), Product_Shape.destroy({ where: { product_id: req.body.id } })]).then((result) => {
                        Promise.all([Product_Shape.bulkCreate(shapelist), Product_Flavour.bulkCreate(flavourlist)]).then((result1) => {
                            res.send({
                                success: true,
